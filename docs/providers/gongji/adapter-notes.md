...
| `timestamp` | integer/string | 必填；毫秒级 Unix 时间戳 |
| `version` | string | 必填；接口版本，当前详情页通常为 `1.0.0` |
| `sign_str` | string | 可选；启用 RSA 签名模式时发送，简易 Token 模式不发送 |

供应商资源接口要求所有请求都携带 `token`、`timestamp` 和 `version`。供应商签名参考流程为 RSA-SHA256、PKCS#1 v1.5、Base64。Astra 默认使用 Token 模式并发送这三个必填头；只有显式配置 `GONGJI_PRIVATE_KEY_PEM` 时才额外发送签名头 `sign_str`。待签名字符串的形式是：
