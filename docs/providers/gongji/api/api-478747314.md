# 删除对象存储已导入元数据

## OpenAPI Specification

```yaml
openapi: 3.0.1
info:
  title: ''
  description: ''
  version: 1.0.0
paths:
  /api/storage/s3/metadata/delete:
    post:
      summary: 删除对象存储已导入元数据
      deprecated: false
      description: >-
        | 当前版本 | 旧版本 | 是否需要加密 | 是否需要加签 |

        | --- | --- | --- | --- |

        | v1.0.0 | - | 否 | 是 |


        ### **描述**

        删除对象存储中已预热导入的目录或文件，并创建元数据删除任务状态。

        加签详见：[加签流程](https://suanli.cn/docs/platform/openapi/m3p6whioxidzwaksughc4gfhnro/#45-%E5%8A%A0%E7%AD%BE%E6%B5%81%E7%A8%8B)
      tags:
        - 共绩算力 Open API/存储/对象存储
        - 对象存储
      parameters:
        - name: token
          in: header
          description: 请填入您在平台内创建的 API 密钥，获取路径为：右上角头像 → API 密钥。
          required: true
          example: a69abd35-1df0-4c21-9c2b-82a20a72d3b2-20260417200925
          schema:
            type: string
        - name: timestamp
          in: header
          description: 时间戳
          required: true
          example: '{{$date.millisecondsTimestamp}}'
          schema:
            type: integer
        - name: version
          in: header
          description: 固定值
          required: true
          example: 1.0.0
          schema:
            type: string
        - name: sign_str
          in: header
          description: 如果token为简易模式则无需填写此字段
          required: false
          example: ''
          schema:
            type: string
      requestBody:
        content:
          application/json:
            schema:
              type: object
              properties:
                storage_id:
                  type: integer
                  format: int64
                  title: 对象存储 ID
                items:
                  type: array
                  title: 删除对象列表
                  items:
                    type: object
                    properties:
                      path:
                        type: string
                        title: 路径
                      type:
                        type: string
                        title: 类型
                        description: type：节点类型；directory（目录）、file（文件）。
                    x-apifox-refs: {}
                    x-apifox-orders:
                      - path
                      - type
                    required:
                      - path
                      - type
                    x-apifox-ignore-properties: []
              x-apifox-refs: {}
              x-apifox-orders:
                - storage_id
                - items
              required:
                - storage_id
                - items
              x-apifox-ignore-properties: []
            example:
              storage_id: 1001
              items:
                - path: dataset/images
                  type: directory
      responses:
        '200':
          description: ''
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/NilResponseEnvelope'
              example:
                code: '0000'
                message: success
                data: null
          headers: {}
          x-apifox-name: NilResponse
      security: []
      x-apifox-folder: 共绩算力 Open API/存储/对象存储
      x-apifox-status: released
      x-run-in-apifox: https://app.apifox.com/web/project/3025695/apis/api-478747314-run
components:
  schemas:
    NilResponseEnvelope:
      type: object
      properties:
        data:
          type: object
          x-apifox-orders: []
          properties: {}
          x-apifox-ignore-properties: []
          nullable: true
        code:
          $ref: '#/components/schemas/ResponseCode'
        message:
          type: string
          title: 响应信息
          nullable: true
      required:
        - data
        - code
        - message
      x-apifox-orders:
        - data
        - code
        - message
      x-apifox-ignore-properties: []
      x-apifox-folder: ''
    ResponseCode:
      type: string
      title: 响应码
      description: 当 code 不为 0000 时，data 为 null。
      enum:
        - '0000'
        - C999
        - C001
        - C002
        - C004
        - C005
        - C006
        - C007
        - C008
        - C009
        - C010
        - Z001
      x-apifox-folder: ''
  securitySchemes:
    AdminAuth:
      type: jwt
      scheme: bearer
      bearerFormat: JWT
      description: 运营中台管理员登录态；需具备权限 `tenant:create_offline_metal_order`
servers:
  - url: https://openapi.suanli.cn
    description: 正式环境
security: []

```
