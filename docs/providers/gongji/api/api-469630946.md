# Job任务队列列表查询接口

## OpenAPI Specification

```yaml
openapi: 3.0.1
info:
  title: ''
  description: ''
  version: 1.0.0
paths:
  /api/job/queue/search:
    get:
      summary: Job任务队列列表查询接口
      deprecated: false
      description: >-
        | 当前版本 | 旧版本 | 是否需要加密 | 是否需要加签 |

        | --- | --- | --- | --- |

        | v1.0.0 | - | 否 | 是 |


        ### **描述**

        获取Job任务队列列表及其运行、计费、任务组容量等信息。

        支持按队列状态、队列名查询。

        加签详见：[加签流程](https://suanli.cn/docs/platform/openapi/m3p6whioxidzwaksughc4gfhnro/#45-%E5%8A%A0%E7%AD%BE%E6%B5%81%E7%A8%8B)
      tags:
        - 共绩算力 Open API/Job批处理任务队列
        - 共绩算力 Open API/任务
      parameters:
        - name: status
          in: query
          description: 队列状态。如果有多个值，请使用英文逗号（,）分隔
          required: false
          example: Pending,Running
          schema:
            type: string
            enum:
              - Pending
              - Running
              - End
            x-apifox-enum:
              - value: Pending
                name: 等待中
                description: ''
              - value: Running
                name: 运行中
                description: ''
              - value: End
                name: 已删除
                description: ''
        - name: search_value
          in: query
          description: 队列名
          required: false
          schema:
            type: string
        - name: page
          in: query
          description: 页码
          required: false
          example: 1
          schema:
            type: integer
        - name: page_size
          in: query
          description: 每页数量
          required: false
          example: 10
          schema:
            type: integer
        - name: token
          in: header
          description: 请填入您在平台内创建的 API 密钥，获取路径为：右上角头像 → API 密钥。
          required: true
          example: ''
          schema:
            type: string
        - name: timestamp
          in: header
          description: 时间戳
          required: true
          example: '{{$date.millisecondsTimestamp}}'
          schema:
            type: integer
        - name: version
          in: header
          description: 固定值
          required: true
          example: 1.0.0
          schema:
            type: string
        - name: sign_str
          in: header
          description: 如果token为简易模式则无需填写此字段
          required: false
          example: ''
          schema:
            type: string
      responses:
        '200':
          description: ''
          content:
            application/json:
              schema:
                type: object
                properties:
                  data:
                    type: object
                    properties:
                      count:
                        type: integer
                      results:
                        type: array
                        items:
                          type: object
                          properties:
                            queue_id:
                              type: integer
                              title: 队列id
                            queue_name:
                              type: string
                              title: 队列名
                            status:
                              type: string
                              title: 队列状态
                              enum:
                                - Pending
                                - Running
                                - End
                              x-apifox-enum:
                                - value: Pending
                                  name: 等待中
                                  description: ''
                                - value: Running
                                  name: 运行中
                                  description: ''
                                - value: End
                                  name: 已删除
                                  description: ''
                            pod_parallel_limit:
                              type: integer
                              title: 队列节点并发上限
                              description: 该队列能够同时运行的最大节点总数
                            group_timeout_sec:
                              type: integer
                              title: 任务组等待超时时间（分钟）
                              description: >-
                                若单个任务组内的所有任务处于等待状态超过设定时间，系统将强制停止该任务组，释放后执行下一组。抢占式实例不受任务组等待超时限制。
                              nullable: true
                            group_capacity:
                              type: object
                              properties:
                                Running:
                                  type: integer
                                  title: 运行中任务组大小
                                Waiting:
                                  type: integer
                                  title: 等待中任务组大小
                                End:
                                  type: integer
                                  title: 已停止任务组大小
                              x-apifox-orders:
                                - Running
                                - Waiting
                                - End
                              title: 组状态队列大小
                            running_task_count:
                              type: integer
                              title: 运行中任务总数
                            current_max_pod_limit:
                              type: integer
                              title: 运行中&待执行最大节点上限
                            create_time:
                              type: string
                              title: 创建时间
                            billing_value:
                              type: integer
                              title: 累计费用(积分)
                          x-apifox-refs: {}
                          x-apifox-orders:
                            - queue_id
                            - queue_name
                            - status
                            - pod_parallel_limit
                            - group_timeout_sec
                            - group_capacity
                            - running_task_count
                            - current_max_pod_limit
                            - billing_value
                            - create_time
                          required:
                            - queue_id
                            - queue_name
                            - status
                            - pod_parallel_limit
                            - group_capacity
                            - running_task_count
                            - current_max_pod_limit
                            - create_time
                            - billing_value
                    x-apifox-orders:
                      - count
                      - results
                    required:
                      - count
                      - results
                    nullable: true
                  code:
                    type: string
                    enum:
                      - '0000'
                      - C999
                      - C001
                      - C002
                      - C004
                      - C005
                      - C006
                      - C007
                      - C008
                      - C009
                      - C010
                      - Z001
                    description: 当code≠0000时，data必为null。
                    title: 响应码
                  message:
                    type: string
                    title: 响应信息
                    nullable: true
                required:
                  - data
                  - code
                  - message
                x-apifox-orders:
                  - code
                  - message
                  - data
                x-apifox-refs: {}
          headers: {}
          x-apifox-name: 成功
      security: []
      x-apifox-folder: 共绩算力 Open API/Job批处理任务队列
      x-apifox-status: released
      x-run-in-apifox: https://app.apifox.com/web/project/3025695/apis/api-469630946-run
components:
  schemas: {}
  securitySchemes:
    AdminAuth:
      type: jwt
      scheme: bearer
      bearerFormat: JWT
      description: 运营中台管理员登录态；需具备权限 `tenant:create_offline_metal_order`
servers:
  - url: https://openapi.suanli.cn
    description: 正式环境
security: []

```
