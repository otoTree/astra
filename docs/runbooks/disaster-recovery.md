...
| Kafka/Redpanda | 允许延迟或重放 | 30 分钟 | PostgreSQL Outbox 可重放事件 |
...
6. 恢复 Kafka/Redpanda，启动 Event Relay，从 Outbox 发布并以事件 ID 去重；检查 backlog、age、retry、DLQ。
...
- PostgreSQL 是唯一状态真源，Redis/Kafka/Provider 状态不得直接覆盖 Task、Attempt 或 Lease。
...
- Kafka 延迟/重复消息，验证 Outbox 重放、Consumer 去重和 DLQ 恢复。
